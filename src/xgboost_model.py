...
def process_combined_df():
    ...
    predictions, actual_values = train_and_evaluate_xgboost(combined_df, target_cols)
...
def process_timeseries_dataframe():
    ...
    # Train and evaluate model
    predictions, actual_values = train_and_evaluate_xgboost(timeseries_dataframe, target_cols)
...
def plot_results(predictions, actual_values, dates, data_type):
    ...
    plt.title(f'XGBoost Predictions vs Actual Values ({data_type})')
    plt.xlabel('Date')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    plt.savefig(f'plots/xgboost_{data_type}_predictions.png')
    plt.close()
...
def save_results(metrics, predictions, actual_values, dates, data_type):
    ...
    metrics_df.to_csv(f'output/xgboost_{data_type}_metrics.csv', index=False)
    ...
    results_df.to_csv(f'output/xgboost_{data_type}_results.csv', index=False)

def main():
    """Main execution function"""
    print("\nProcessing combined_df with XGBoost...")
    ...
    print("\nProcessing timeseries_dataframe with XGBoost...")
